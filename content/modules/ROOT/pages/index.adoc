...
=== Verify contexts and clusters

First, let's verify that the contexts and clusters are configured correctly.

*Procedure*

[start=1]
. Switch to the *Bastion* tab, located on the right side of the screen.
. Verify that both contexts appear:
+
...
The setup script configures required bastion environment variables (including RHACS client settings) before running the rest of the install. This will also deploy a bunch of insecure applications to the local cluster and ensure the lab is set up correctly.
